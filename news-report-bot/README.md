...
1. **윈도우 PC**여야 한다 (아웃룩 자동화가 윈도우 전용 기능이다)
2. 실행 전에 **아웃룩(클래식)에 로그인**되어 있어야 한다 — "새 아웃룩(New Outlook)"만 켜져
   있으면 안 된다. 왼쪽 위에 파일 탭이 있는 예전 화면인지 확인할 것
...
| 증상 | 원인과 해결 |
|---|---|
| `아웃룩 발송 실패` | 아웃룩이 꺼져 있거나 로그인이 안 된 경우가 대부분. 아웃룩을 먼저 켜서 로그인 화면이 없는지 확인 |
