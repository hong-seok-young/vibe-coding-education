...
- **① 이번 시간** — 이 단계의 목표와 결과물을 짧게 보여주고, 만들 결과물이 **동작하는 상태로** 바로 아래
  들어 있다. 프롬프트를 치기 전에 무엇을 만드는지 눈으로 확인하는 자리다.
- **② 프롬프트** — 그대로 복사해 AI 대화창(Claude Code / Cursor / 사내 AI)에 붙여넣는다.
- **③ 샘플 결과** — AI 응답을 기다리기 싫거나 실습 없이 흐름만 보고 싶을 때. 그 프롬프트를 실행하면
  나올 결과(코드·문서·표)를 미리 넣어 두었다. 이 탭만 읽어도 전체 과정을 이해할 수 있다.
- **④ 원리** — dX팀이 대신 만들어 준다 해도 원리는 알아야 하므로, 왜 그렇게 하는지와
  무엇을 결정해야 하는지를 다룬다. (OAuth, 증분 동기화, 권한 검사, 마이그레이션, RADIUS, 컨테이너 배포 등)
- **⑤ 확인** — 체크리스트로 스스로 점검하고 단계를 완료로 표시한다.

각 탭 아래에는 항상 **"다음 · N. ○○ →"** 버튼이 있어서, 교육생이 순서를 놓치지 않고 따라올 수 있습니다.
...
| STEP 06 구글 캘린더 | **실제 구글 캘린더 연동 테스트** (아래 참고) |
| STEP 07 다른 캘린더 추가 | 캘린더 연동 설정 (프로바이더 능력 기반 UI) |
| STEP 08 주간보고 | 주간보고 |
| STEP 09 발표 모드 | 발표 모드 |
| STEP 10 인수인계 | 인수인계 |
...
STEP 06 의 데모는 목업이 아니라 **학습자 본인의 구글 캘린더에 실제로 연결**됩니다.
전체 동기화 → 구글에서 일정 변경 → 증분 동기화(변경분만) → 원격 삭제 반영 → 내보내기까지
직접 눌러보면서, 이 단계가 글로 설명하는 내용을 눈으로 확인할 수 있습니다.

**학습자가 준비할 것** (1회, 약 5분) — 화면의 "준비 절차" 접힘 상자에 같은 내용이 있습니다.

1. 구글 클라우드 콘솔에서 프로젝트 생성
2. **Google Calendar API** 활성화
3. OAuth 동의 화면 구성 — 외부 유형이면 **테스트 사용자에 본인 계정 추가** (안 하면 동의 화면에서 막힘)
4. OAuth 클라이언트 ID(웹 애플리케이션) 생성 →
   **승인된 JavaScript 원본**에 `https://<계정>.github.io` 입력
   (저장소 경로 `/vibe-coding-education` 은 넣지 않습니다. 경로는 오리진의 일부가 아닙니다)
5. 만들어진 클라이언트 ID를 데모 화면에 붙여넣기 (이 브라우저에만 저장됩니다)

팀 공용 클라이언트 ID를 미리 채워두려면 저장소에 변수를 하나 넣으면 됩니다.
클라이언트 ID는 브라우저에 노출되는 공개 값이므로 secret 이 아니라 **variable** 로 둡니다.

> Settings → Secrets and variables → Actions → Variables → `GOOGLE_CLIENT_ID`

**이 데모의 한계 (교육상 중요)**

정적 사이트라 서버가 없으므로, 구글이 자바스크립트 앱에 제공하는 **브라우저 토큰 흐름**을 씁니다.
STEP 06 이 가르치는 정식 서버 흐름과 다르고, 그 차이가 곧 "왜 실제 앱에는 서버가 필요한가"의 답입니다.

| 항목 | 서버 흐름 (실제 앱) | 브라우저 흐름 (이 데모) |
|---|---|---|
| 클라이언트 시크릿 | 서버에만 보관 | 쓰지 않음 (클라이언트 ID만) |
| `refresh_token` | 받아서 암호화 저장 | **받을 수 없음** |
| 토큰 보관 | DB에 암호화 | 메모리만 — 새로고침하면 재연결 |
| 자동 주기 동기화 | 가능 | **불가능** — 화면을 열고 눌러야 함 |
| 요청 scope | `calendar.events` | 읽기는 `calendar.events.readonly` (더 좁게) |

데모는 사용자의 기본 캘린더만 읽고, 액세스 토큰을 브라우저 저장소에 남기지 않습니다.
내보내기 실습은 `[교육 데모]` 접두사가 붙은 일정을 만들고, 화면의 정리 버튼으로 지울 수 있습니다.
...
│  └─ google/              구글 연동 계층
│     ├─ gis.ts            OAuth 토큰 요청·회수 (브라우저 흐름)
│     ├─ calendar.ts       Calendar API v3 호출 · 증분 동기화 · 정규화
│     └─ useGoogleLive.ts  연결·동기화 상태와 기록
