    ...
    sample: [
      {
        type: 'callout',
        tone: 'tip',
        title: '이 단계는 읽지 말고 직접 붙여볼 수 있다',
        text: '**완성 데모** 탭에서 본인 구글 계정으로 실제 연결해볼 수 있다. 구글 클라우드 콘솔에서 클라이언트 ID 하나만 만들면(5분), 아래에 적힌 전체 동기화 · 증분 동기화 · 삭제 반영 · 내보내기를 눈으로 확인할 수 있다. 아래 코드를 읽기 전에 먼저 해보면 무슨 말인지 훨씬 빨리 이해된다.',
      },
...
async function callGoogle(path: string, token: string, init: RequestInit = {}, attempt = 0): Promise<Response> {
  ...
  return stats
    ...
    explain: [
      ...
      { type: 'h', text: '데모는 왜 서버 흐름이 아닌가' },
      {
        type: 'p',
        text: '이 교육 프로그램 자체는 서버가 없는 정적 사이트다. 그래서 **완성 데모** 탭의 실연동은 구글이 자바스크립트 앱에 제공하는 토큰 흐름을 쓴다. 위에서 설명한 서버 흐름과 무엇이 다른지 알고 보면, 왜 실제 앱에 서버가 필요한지가 분명해진다.',
      },
      {
        type: 'table',
        head: ['항목', '서버 흐름 (실제 앱 · 위 코드)', '브라우저 흐름 (데모)'],
        rows: [
          ['클라이언트 시크릿', '서버에만 보관', '쓰지 않음 (클라이언트 ID만)'],
          ['`refresh_token`', '받아서 암호화 저장', '**받을 수 없다** — 인가 코드를 토큰으로 교환할 때 시크릿이 필요하고, 브라우저에는 시크릿을 둘 수 없다'],
          ['토큰 보관', 'DB에 암호화', '메모리만. 새로고침하면 사라진다'],
          ['자동 주기 동기화', '가능 (서버 스케줄러)', '**불가능** — 사용자가 화면을 열고 버튼을 눌러야 한다'],
          ['요청 scope', '`calendar.events`', '읽기는 `calendar.events.readonly` 로 더 좁게'],
        ],
      },
      {
        type: 'callout',
        tone: 'warn',
        title: '데모가 만들 수 있는 오해',
        text: '"브라우저에서 되니까 서버 없이 만들면 되겠다"는 결론은 틀렸다. 브라우저 흐름으로는 **사용자가 앱을 열어두지 않으면 아무 일도 일어나지 않는다.** 아침에 캘린더가 자동으로 맞춰져 있어야 하는 우리 요구사항은 서버 흐름이 아니면 성립하지 않는다. 데모는 "OAuth와 동기화가 어떻게 동작하는가"를 보여주는 것이고, 실제 구조는 위 코드가 맞다.',
      },
      {
        type: 'callout',
        tone: 'dx',
    ...
    checklist: [
      '**완성 데모** 탭에서 내 구글 계정으로 실제 연결해 전체 동기화를 해봤다',
      '구글 캘린더에서 일정을 바꾼 뒤 증분 동기화를 눌러 "변경분만" 잡히는 것을 확인했다',
      '구글에서 일정을 지운 뒤 동기화하니 데모에서도 사라지는 것을 확인했다',
    ...
    demo: {
      kind: 'google-live',
      hint: '**읽기만 하지 말고 직접 붙여보라.** 본인 구글 계정으로 연결해서 전체 동기화 → 구글에서 일정을 바꾸고 증분 동기화 → 삭제 반영 → 내보내기까지 실제로 확인할 수 있다. 이 단계가 글로 설명하는 "증분 동기화가 왜 싼가", "왜 중복이 안 생기는가", "syncToken 이 만료되면 어떻게 되는가"를 눈으로 보는 것이 목표다.',
    },
  },
